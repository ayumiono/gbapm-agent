...
public class AgentInfoQueueDataSender extends QueueDataSender {
	
	private final Logger agentInfoListenerLogger = LoggerFactory.getLogger("agentInfoListenerLogger");
	...
	private void initAppender() {
		try {
			SQLLOG_FILE = config.readString("profiler.agent.info.log", "/tmp/asm.log");
			...
			if (agentInfoListenerLogger instanceof ch.qos.logback.classic.Logger) {
				ch.qos.logback.classic.Logger _logger = (ch.qos.logback.classic.Logger) agentInfoListenerLogger;
	...
	public AgentInfoQueueDataSender(InvokeStackEvent interest,ProfilerConfig config) {
		super(interest);
		this.config = config;
	}
	...
	public boolean deal(APMModel frame) {
		try {
			...
			agentInfoListenerLogger.info(json);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
