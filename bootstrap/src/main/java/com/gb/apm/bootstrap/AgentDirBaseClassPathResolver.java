...
/**
 * 
 * gbapm-bootstrap.jar通过-agent参数指定
 * 通过jvm.class.path系统参数获取gbapm-bootstrap.jar所在目录
 * 找到所在目录下的boot目录
 * 找到
 * ../boot/gbapm-bootstrap-core.jar
 * ../boot/gbapm-commons.jar
 * ../boot/gbapm-annotations.jar	optional dependency
 * 
 * @author xuelong.chen
 *
 */
public class AgentDirBaseClassPathResolver implements ClassPathResolver {
    ...
    static final String VERSION_PATTERN = "(-[0-9]+(\\.[0-9]+)+((\\-SNAPSHOT)|(-RC[0-9]+))?)?";
    static final Pattern DEFAULT_AGENT_PATTERN = compile("gbapm-bootstrap" + VERSION_PATTERN + "\\.jar");
    static final Pattern DEFAULT_AGENT_COMMONS_PATTERN = compile("gbapm-common" + VERSION_PATTERN + "\\.jar");
    static final Pattern DEFAULT_AGENT_CORE_PATTERN = compile("gbapm-bootstrap-core" + VERSION_PATTERN + "\\.jar");
...
//    	return System.getProperty("java.class.path");FIXME
        return System.getProperty("java.class.path")
//        		+";D:\\workspace\\apm\\distribute\\target\\gbapm-agent-0.0.1-SNAPSHOT\\gbapm-agent\\gbapm-bootstrap-0.0.1-SNAPSHOT.jar"
        		;
    }
    ...
    boolean findAgentJar() {
        ...
        this.pinpointCommonsJar = findFromBootDir("gbapm-common.jar", agentCommonsPattern);
        this.bootStrapCoreJar = findFromBootDir("gbapm-bootstrap-core.jar", agentCorePattern);
        return true;
    }
