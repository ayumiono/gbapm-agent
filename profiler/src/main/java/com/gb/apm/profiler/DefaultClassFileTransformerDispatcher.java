...
import com.gb.apm.asm.instrument.ClassFileFilter;
import com.gb.apm.asm.instrument.PinpointClassFilter;
import com.gb.apm.asm.instrument.UnmodifiableClassFilter;
...
import com.gb.apm.profiler.instrument.transformer.DebugTransformerRegistry;
import com.gb.apm.profiler.instrument.transformer.DefaultTransformerRegistry;
import com.gb.apm.profiler.instrument.transformer.TransformerRegistry;
import com.gb.apm.profiler.plugin.MatchableClassFileTransformer;
...
public class DefaultClassFileTransformerDispatcher implements ClassFileTransformerDispatcher {
    ...
    public byte[] transform(ClassLoader classLoader, String classInternalName, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classFileBuffer) throws IllegalClassFormatException {
        ...
        ClassFileTransformer transformer = this.transformerRegistry.findTransformer(JavaAssistUtils.jvmNameToJavaName(classInternalName));
        if (transformer == null) {
            // For debug
